...
import { useNotificationsQuery, useMarkNotificationReadMutation } from '../../hooks/notificationQuery'
...
// Đọc 1 thông báo gọi PATCH /notifications/:id/read (useMarkNotificationReadMutation) — readOverrides
// chỉ dùng để cập nhật UI ngay lập tức (optimistic) trong lúc chờ request, sau đó query được
// invalidate nên data thật từ server sẽ ghi đè lại. "Mark all read" chưa có endpoint bulk riêng nên
// gọi lặp mutation cho từng thông báo đang chưa đọc.
export default function NotificationsPage() {
  ...
  const handleItemClick = (n: Notification) => {
    if (!n.isRead) {
      setReadOverrides((prev) => new Set(prev).add(n.id))
      markAsRead(n.id)
    }
